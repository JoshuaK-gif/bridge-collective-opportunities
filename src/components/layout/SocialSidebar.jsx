...
export default function SocialSidebar() {
  ...
  return (
    <div className="hidden lg:flex fixed left-0 top-1/2 -translate-y-1/2 z-40 flex-col">
      <button
        onClick={() => setExpanded(!expanded)}
        className="w-10 h-10 bg-primary text-primary-foreground flex items-center justify-center rounded-r-lg shadow-lg hover:opacity-90 transition-all"
        title="Share"
      >
        <Share2 className="w-4 h-4" />
      </button>
      {expanded && (
        <div className="flex flex-col bg-card border border-l-0 rounded-r-lg shadow-lg overflow-hidden">
          {socials.map(s => (
            <a
              key={s.name}
              href={s.href}
